...
public:
	...
	bool isStatic; /* tells if the object is immovable for collision resolution purposes */
...
private:
	...
	glm::vec3 lastFrameAcceleration; /* linear acceleration in world space during the last frame */
	...
	glm::mat3 inertiaTensor; /* moment of inertia tensor in body's local space, with mass factored out */
	glm::mat3 invInertiaTensorWorld; /* inverse inertia tensor in world space, used in calculations */

public:
	RigidBody(
		const glm::vec3 & position = glm::vec3(0),
		const glm::vec3 & scale = glm::vec3(1),
		const glm::quat & orientation = glm::normalize(glm::angleAxis(0.0f, glm::vec3(0, 1, 0)))
	);

	/* updates the transform matrix from state data(position, orientation, scale) */
	void updateTransformMatrix();
	/* updates the inverse inertia tensor in world coordinates from state data(position, orientation) */
