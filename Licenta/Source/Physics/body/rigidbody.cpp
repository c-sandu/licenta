...
RigidBody::RigidBody(const glm::vec3& position,
...
void RigidBody::updateTransformMatrix() {
  transform = glm::translate(glm::mat4(1), position) *
              glm::mat4_cast(orientation) * glm::scale(glm::mat4(1), scale) *
              glm::mat4(1);
}
...
void RigidBody::integrate(float deltaTime) {
  if ((invMass > PhysicsSettings::get().epsilons.globalEpsilon) && isAwake) {
    ...
    orientation = orientation +
                  deltaTime * 0.5f * glm::quat(0.0f, angVelocity) * orientation;
    ...
    float currentMotion =
        glm::dot(linVelocity, linVelocity) + glm::dot(angVelocity, angVelocity);
    float baseBias = 0.8f;
    float bias = glm::pow(baseBias, deltaTime);
    motion = bias * motion + (1.0f - bias) * currentMotion;
    glm::clamp(motion, 0.0f,
               10.0f * PhysicsSettings::get().rigidBodies.sleepMotionThreshold);
  }
}

void RigidBody::applyForce(const glm::vec3& force) {
  glm::vec3 adjustedForce =
      force * PhysicsSettings::get().forceMultipliers.force;
  forceAccumulator += adjustedForce;
}

void RigidBody::applyForceAtWorldPoint(const glm::vec3& force,
...
void RigidBody::applyForceAtLocalPoint(const glm::vec3& force,
                                       const glm::vec3& point) {
  glm::vec3 adjustedForce =
      force * PhysicsSettings::get().forceMultipliers.force;
...
void RigidBody::applyTorqueAtLocalPoint(const glm::vec3& torque,
                                        const glm::vec3& point) {
  glm::vec3 adjustedTorque =
      torque * PhysicsSettings::get().forceMultipliers.torque;
  glm::vec3 pointWorld = transform * glm::vec4(point, 1);
...
void RigidBody::applyLinearImpulse(const glm::vec3& impulse) {
  isAwake = true;
  glm::vec3 adjustedImpulse =
      impulse * PhysicsSettings::get().forceMultipliers.impulse;
...
glm::mat3 RigidBody::inertiaTensorCube(const glm::vec3& halfSizes) {
  ...
  return 1.0f / 3.0f *
         glm::mat3(halfSizes.y * halfSizes.y + halfSizes.z * halfSizes.z, 0, 0,
...
glm::mat3 RigidBody::inertiaTensorSphere(const float radius) {
  /* mass is factored out */
  return 2.0f / 5.0f *
         glm::mat3(radius * radius, 0, 0, 0, radius * radius, 0, 0, 0,
                   radius * radius);
}

glm::mat3 RigidBody::inertiaTensorCylinder(const float height,
                                           const float radius) {
  /* mass is factored out */
  return 1.0f / 12.0f *
         glm::mat3(3.0f * radius * radius + height * height, 0, 0, 0,
                   6.0f * radius * radius, 0, 0, 0,
                   3.0f * radius * radius + height * height);
}

glm::mat3 RigidBody::inertiaTensorCone(const float height, const float radius) {
  return 1.0f / 20.0f *
         glm::mat3(12.0f * height * height + 3.0f * radius * radius, 0, 0, 0,
                   6.0f * radius * radius, 0, 0, 0,
                   12.0f * height * height + 3.0f * radius * radius);
}

glm::mat3 RigidBody::inertiaTensorCapsule(const float height,
                                          const float radius) {
...
std::string RigidBody::toString() {
  return std::string("") + "\tRigidBody {" + "\n\t\t" +
         ...
         "scale = " + to_string(scale) + ", mass = " + std::to_string(mass) +
         ", invMass = " + std::to_string(invMass) + "\n\t\t" +
