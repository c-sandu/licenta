...
glm::vec3 Plane::getSupportPtInLocalSpace(glm::vec3 dir) {
  return glm::vec3(rx * (dir.x >= 0.0f ? 1.0f : -1.0f), 0,
                   rz * (dir.z >= 0.0f ? 1.0f : -1.0f));
}

glm::vec3 Box::getSupportPtInLocalSpace(glm::vec3 dir) {
  return glm::vec3(halfSizes.x * glm::sign(dir.x),
...
glm::vec3 Cylinder::getSupportPtInLocalSpace(glm::vec3 dir) {
  glm::vec3 supportDisc = glm::length(glm::vec3(dir.x, 0, dir.z)) > 0
                              ? radius * glm::vec3(dir.x, 0, dir.z) /
                                    glm::length(glm::vec3(dir.x, 0, dir.z))
                              : glm::vec3(0);
