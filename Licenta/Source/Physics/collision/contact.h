...
class CollisionPoint {
 public:
  glm::vec3
      points[2]; /* contact points on the 2 objects in world coordinates */
  glm::vec3
      normal; /* collision normal (from first object to the second object) */
  ...
  CollisionPoint(const glm::vec3 point1,
                 const glm::vec3 point2,
                 const glm::vec3 normal,
                 const float penetration)
      : points{point1, point2}, normal(normal), penetration(penetration) {}
...
class Contact : public CollisionPoint {
 ...
 private:
  glm::vec3 localPoints[2]; /* points in the individual objects' local spaces */
                            /* derived data below */
 public:
  glm::mat3 matContactToWorld; /* transform matrix from contact space to world
                                  space */
 private:
  glm::mat3 matWorldToContact; /* transform matrix from world space to contact
                                  space */
 ...
 public:
  ...
  Contact(const CollisionPoint& collisionPoint) {
    ...
    this->localPoints[0] = glm::vec3(
        glm::inverse(collisionPoint.objects[0]->getTransformMatrix()) *
        glm::vec4(this->points[0], 1));
    this->localPoints[1] = glm::vec3(
        glm::inverse(collisionPoint.objects[1]->getTransformMatrix()) *
    ...
    this->frictionCoef =
        glm::mix(std::min(objects[0]->body->frictionCoef,
                          objects[1]->body->frictionCoef),
                 std::max(objects[0]->body->frictionCoef,
                          objects[1]->body->frictionCoef),
                 PhysicsSettings::get().collisionResolution.coefInterpAlpha);
    this->invalid = false;
  ...
  void setContactInfo(const glm::vec3 point1,
                      const glm::vec3 point2,
                      const glm::vec3 normal,
                      const float penetration,
                      PhysicsObject* obj1,
