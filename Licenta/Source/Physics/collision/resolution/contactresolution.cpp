...
float SequentialImpulseContactResolver::computeDesiredDeltaVelocity(
    Contact* contact) {
  /* velocity built from acceleration should be removed to prevent bounciness of
  resting contacts before correction: v_sep = -(1 + restitution) * v_closing
  after correction:  v_sep = -v_acc - (1 + restitution) * (v_closing - v_acc) */
  float velocityFromAcceleration = 0;

  const glm::vec3& lastFrameAccelerationA =
      contact->objects[0]->body != nullptr
          ? contact->objects[0]->body->lastFrameAcceleration
          : glm::vec3(0);
  const glm::vec3& lastFrameAccelerationB =
      contact->objects[1]->body != nullptr
          ? contact->objects[1]->body->lastFrameAcceleration
          : glm::vec3(0);

  /* final velocity from acceleration, in the direction of the contact's normal
   */
  velocityFromAcceleration = glm::dot(lastFrameAccelerationA, contact->normal) -
                             glm::dot(lastFrameAccelerationB, contact->normal);

  float effectiveRestitution = 0.0f;
  if (contact->closingVelocityContact.x - velocityFromAcceleration >=
      PhysicsSettings::get().collisionResolution.minVelocityForRestitution) {
    /* apply restitution only if the remaining closing velocity is greater than
     * a threshold */
    effectiveRestitution = contact->restitutionCoef;
  }

  return -velocityFromAcceleration -
         (1.0f + effectiveRestitution) *
             (contact->closingVelocityContact.x - velocityFromAcceleration);
}
...
glm::vec3 SequentialImpulseContactResolver::computeImpulse(Contact* contact) {
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    /* instead of solving for each axis separately, use the matrix cross product
     * trick */
    /* impulseToTorque is actually holding all the (r_a cross n) products from
     * the above formula for the 3 different contact basis axes:*/
    glm::mat3 impulseToTorque =
        glm::matrixCross3(contact->relativeContactPositions[i]);
    glm::mat3 tmpRotationalInertiaWorld =
        impulseToTorque * contact->objects[i]->body->invInertiaTensorWorld;
    tmpRotationalInertiaWorld = -tmpRotationalInertiaWorld * impulseToTorque;
    totalRotationalInertiaWorld += tmpRotationalInertiaWorld;
  }

  /* transform to contact space to add the linear component along each of the 3
   * axes */
  glm::mat3 totalInertiaContact =
      (contact->matWorldToContact * totalRotationalInertiaWorld) *
      contact->matContactToWorld;
  ...
  const glm::vec3 velocityToKill = glm::vec3(
      contact->desiredDeltaVelocity, -contact->closingVelocityContact.y,
      -contact->closingVelocityContact.z);
  ...
  const float planarImpulse = glm::sqrt(impulseContact.y * impulseContact.y +
                                        impulseContact.z * impulseContact.z);
  /* check if we have dynamic friction  */
  if (planarImpulse > impulseContact.x * contact->frictionCoef &&
      planarImpulse > PhysicsSettings::get().epsilons.globalEpsilon) {
...
void SequentialImpulseContactResolver::applyPositionUpdate(
  ...
  for (uint8_t i = 0; i < 2; i++) {
    if (deepestContact->objects[i]->body == nullptr ||
        deepestContact->objects[i]->body->isStatic)
      continue;

    /* angInertia = dot(r_a cross n, invI_a^-1 * (r_a cross n)) */
    angularInertias[i] =
        glm::dot(glm::cross(deepestContact->relativeContactPositions[i],
                            deepestContact->normal),
                 deepestContact->objects[i]->body->invInertiaTensorWorld *
                     glm::cross(deepestContact->relativeContactPositions[i],
                                deepestContact->normal));
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    /* restrict angular movement for stability reasons -- smaller objects should
     * have smaller rotation */
    /* limit wrt to the length of the relative contact position as an
     * approximation of object's size */
    float angularMovementLimit =
        PhysicsSettings::get().collisionResolution.angularMovementLimitFactor *
        glm::length(deepestContact->relativeContactPositions[i]);
    if (glm::abs(angularMovement[i]) > angularMovementLimit) {
      float totalMove = linearMovement[i] + angularMovement[i];
      angularMovement[i] = glm::clamp(angularMovement[i], -angularMovementLimit,
                                      angularMovementLimit);
    ...
    glm::vec3 rotationDirection = glm::cross(
        deepestContact->relativeContactPositions[i], deepestContact->normal);
    angularDelta = (deepestContact->objects[i]->body->invInertiaTensorWorld *
                    rotationDirection) *
                   (angularMovement[i] / angularInertias[i]);

    /* apply the changes to the object */
    deepestContact->objects[i]->body->position += linearDelta;
    glm::quat aux = glm::quat(0, angularDelta);
    aux *= deepestContact->objects[i]->body->orientation;
    aux = deepestContact->objects[i]->body->orientation + 0.5f * aux;
    deepestContact->objects[i]->body->orientation = glm::normalize(aux);

    /* apply the changes to this contact itself and all the other contacts in
     * the manifold */
    for (auto& contact : manifold.contacts) {
      const uint8_t colliderIdxInOtherContact =
          contact->objects[0] == deepestContact->objects[i] ? 0 : 1;
      ...
      const glm::vec3 deltaPosition =
          ...
          glm::cross(
              angularDelta,
              contact->relativeContactPositions[colliderIdxInOtherContact]);
...
void SequentialImpulseContactResolver::applyVelocityUpdate(
    ContactManifold& manifold,
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    const glm::vec3 velDelta = (i == 0 ? 1.0f : -1.0f) * impulse *
                               fastestContact->objects[i]->body->invMass;
    fastestContact->objects[i]->body->linVelocity += velDelta;

    /* angular velocity delta */
    glm::vec3 angVelDelta =
        fastestContact->objects[i]->body->invInertiaTensorWorld *
        (i == 0 ? 1.0f : -1.0f) *
        glm::cross(fastestContact->relativeContactPositions[i], impulse);
    ...
    for (auto& contact : manifold.contacts) {
      const uint8_t colliderIdxInOtherContact =
          contact->objects[0] == fastestContact->objects[i] ? 0 : 1;
      const float sign = colliderIdxInOtherContact == 0 ? -1.0f : 1.0f;
      ...
      contact->closingVelocityContact =
          contact->matWorldToContact * contact->closingVelocityWorld;
      contact->desiredDeltaVelocity = computeDesiredDeltaVelocity(contact);
    }
  }
}

void SequentialImpulseContactResolver::solveContactManifold(
    ContactManifold& manifold,
    float deltaTime) {
  ...
  for (penetrationIterations = 0;
    ...
    if (deepestContact == nullptr ||
        deepestContact->penetration <
  ...
  /* fix velocities */
  for (velocityIterations = 0;
       velocityIterations <
       PhysicsSettings::get().collisionResolution.velMaxIterations;
       velocityIterations++) {
    /* get the contact with the lowest desired delta velocity (aka the fastest
     * one) */
    Contact* fastestContact = manifold.getFastestContact();

    if (fastestContact == nullptr ||
        fastestContact->desiredDeltaVelocity >=
            PhysicsSettings::get().epsilons.velEpsilon) {
      velocityIterations++;
      break;
    }
...
void SequentialImpulseContactResolver::solve(
    const std::vector<CollisionPoint*>& collisions,
    float deltaTime) {
...
void SequentialImpulseContactResolver::updateContacts(
    const std::vector<CollisionPoint*>& collisions) {
  ...
  for (unsigned int i = 0; i < collisions.size(); i++) {
    ...
    for (auto& manIt : manifolds) {
      if (manIt.obj1 == collision->objects[0] &&
          manIt.obj2 == collision->objects[1]) {
        manifold = &manIt;
      }
    }

    /* if there's no manifold for this pair of objects, try again with reversed
     * order */
    if (manifold == nullptr) {
      for (auto& manIt : manifolds) {
        if (manIt.obj1 == collision->objects[1] &&
            manIt.obj2 == collision->objects[0]) {
          manifold = &manIt;
  ...
  for (auto manIt = manifolds.begin(); manIt != manifolds.end();) {
    ...
    for (uint8_t c = 0; c < manIt->contacts.size();) {
      ...
      if (contact->timestamp < timestamp) {
        /* new world positions */
        glm::vec3 worldPoint1 = glm::vec3(
            obj1->getTransformMatrix() * glm::vec4(contact->localPoints[0], 1));
        glm::vec3 worldPoint2 = glm::vec3(
            obj2->getTransformMatrix() * glm::vec4(contact->localPoints[1], 1));

        /* recompute penetration distance */
        const float penetration =
            glm::dot(worldPoint2 - worldPoint1, contact->normal);
        ...
        longer colliding) or if the distance between the collision points in the
        plane perpendicular to the normal vector becomes too large. */
        if (penetration > PhysicsSettings::get().epsilons.globalEpsilon ||
            tangentDistanceSquared >
                PhysicsSettings::get().epsilons.globalEpsilon) {
          contacts.remove(*(manIt->contacts[c]));
...
void SequentialImpulseContactResolver::addContactToManifold(
    ContactManifold& manifold,
    const CollisionPoint& newContact) {
  ...
  const glm::vec3 localPoint1 = glm::vec3(
      glm::inverse(obj1->getTransformMatrix()) * glm::vec4(worldPoint1, 1));
  const glm::vec3 localPoint2 = glm::vec3(
      glm::inverse(obj2->getTransformMatrix()) * glm::vec4(worldPoint2, 1));
  ...
  for (uint8_t i = 0; i < manifold.contacts.size(); i++) {
    ...
    const int closeEnough = (distance1Squared < toleranceSquared) &
                            (distance2Squared < toleranceSquared);
  ...
  if (manifold.contacts.size() ==
      PhysicsSettings::get().collisionResolution.maxContactsPerManifold) {
...
uint8_t SequentialImpulseContactResolver::findWorstContact(
    ContactManifold& manifold) {
  ...
  const glm::vec3 v0v1 =
      manifold.contacts[0]->points[0] - manifold.contacts[1]->points[0];
  const glm::vec3 v0v2 =
      manifold.contacts[0]->points[0] - manifold.contacts[2]->points[0];
  const glm::vec3 v0v3 =
      manifold.contacts[0]->points[0] - manifold.contacts[3]->points[0];
  const glm::vec3 v1v2 =
      manifold.contacts[1]->points[0] - manifold.contacts[2]->points[0];
  const glm::vec3 v1v3 =
      manifold.contacts[1]->points[0] - manifold.contacts[3]->points[0];
  const glm::vec3 v2v3 =
      manifold.contacts[2]->points[0] - manifold.contacts[3]->points[0];

  float* areas = new float[PhysicsSettings::get()
                               .collisionResolution.maxContactsPerManifold];

  /* Compute the areas for each triangle formed by excluding the vertex at the
   * area index. */
  areas[0] =
      (lowestPenetrationIdx != 0) ? glm::length(glm::cross(v1v2, v1v3)) : 0.0f;
  areas[1] =
      (lowestPenetrationIdx != 1) ? glm::length(glm::cross(v0v2, v0v3)) : 0.0f;
  areas[2] =
      (lowestPenetrationIdx != 2) ? glm::length(glm::cross(v0v1, v0v3)) : 0.0f;
  areas[3] =
      (lowestPenetrationIdx != 3) ? glm::length(glm::cross(v0v1, v0v2)) : 0.0f;
...
Contact* ContactManifold::getDeepestContact() {
  ...
  for (auto& contact : contacts)
    if (deepestContact == nullptr ||
        contact->penetration > deepestContact->penetration)
...
Contact* ContactManifold::getFastestContact() {
  ...
  for (auto& contact : contacts) {
    if (contact->penetration < -PhysicsSettings::get().epsilons.penEpsilon)
      continue;
    if (fastestContact == nullptr ||
        contact->desiredDeltaVelocity < fastestContact->desiredDeltaVelocity)
