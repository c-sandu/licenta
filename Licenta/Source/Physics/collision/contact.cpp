...
/* source: Ian Millington - Game Physics Engine Development (2007) -
 * chapter 14.1.2 */
static glm::mat3 makeOrthonormalBasis(const glm::vec3& contactNormal) {
  ...
  if (glm::abs(contactNormal.x) > glm::abs(contactNormal.y)) {
    const float s = 1.0f / glm::sqrt(contactNormal.z * contactNormal.z +
...
void Contact::setContactInfo(const glm::vec3 point1,
  ...
  this->localPoints[0] = glm::vec3(glm::inverse(obj1->getTransformMatrix()) *
                                   glm::vec4(this->points[0], 1));
  this->localPoints[1] = glm::vec3(glm::inverse(obj2->getTransformMatrix()) *
                                   glm::vec4(this->points[1], 1));
...
void Contact::setContactInfo(const CollisionPoint& contactInfo) {
  ...
  this->localPoints[0] =
      glm::vec3(glm::inverse(contactInfo.objects[0]->getTransformMatrix()) *
                glm::vec4(this->points[0], 1));
  this->localPoints[1] =
      glm::vec3(glm::inverse(contactInfo.objects[1]->getTransformMatrix()) *
                glm::vec4(this->points[1], 1));
...
void Contact::computeDerivedData(float deltaTime) {
  this->restitutionCoef =
      glm::mix(std::min(objects[0]->body->restitutionCoef,
                        objects[1]->body->restitutionCoef),
               std::max(objects[0]->body->restitutionCoef,
                        objects[1]->body->restitutionCoef),
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    glm::vec3 lastFramePlanarAcceleration =
        objects[i]->body->lastFrameAcceleration -
        glm::dot(objects[i]->body->lastFrameAcceleration, normal) * normal;
    glm::vec3 lastFramePlanarVelocityFromAcc =
        lastFramePlanarAcceleration * deltaTime;
    glm::vec3 localVelocityWorld =
        glm::cross(objects[i]->body->angVelocity, relativeContactPositions[i]) +
        objects[i]->body->linVelocity + lastFramePlanarVelocityFromAcc;
...
std::string Contact::toString() {
  return std::string("") + "\tContact {" +
         ...
         "\n\t\t" + "relativeContactPositions = { " +
         to_string(relativeContactPositions[0]) + ", " +
         to_string(relativeContactPositions[1]) + " }\n\n\t\t" +
