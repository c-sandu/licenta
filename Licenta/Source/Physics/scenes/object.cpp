...
glm::mat4 PhysicsObject::getTransformMatrix() const {
  if (body == nullptr)
    return glm::translate(glm::mat4(1), ((PlaneCollider*)collider)->normal *
                                            ((PlaneCollider*)collider)->offset);
...
ObjectSpawner::ObjectSpawner(std::unordered_map<std::string, Mesh*>* meshes,
...
void ObjectSpawner::updateFromCamera(glm::vec3 cameraPosition,
                                     glm::vec3 forward) {
    ...
    if (!settings.is<picojson::null>()) {
      ...
      if (!damping.is<picojson::null>()) {
        ...
        if (!linear.is<picojson::null>())
          PhysicsSettings::get().damping.linear = (float)linear.get<double>();
        picojson::value angular = damping.get("angular");
        if (!angular.is<picojson::null>())
          PhysicsSettings::get().damping.angular = (float)angular.get<double>();
      }
      ...
      if (!forceMultipliers.is<picojson::null>()) {
        ...
        if (!torque.is<picojson::null>())
          PhysicsSettings::get().forceMultipliers.torque = (float)torque.get<double>();
        picojson::value force = forceMultipliers.get("force");
        if (!force.is<picojson::null>())
          PhysicsSettings::get().forceMultipliers.force = (float)force.get<double>();
        picojson::value impulse = forceMultipliers.get("impulse");
        if (!impulse.is<picojson::null>())
          PhysicsSettings::get().forceMultipliers.impulse =
              (float)impulse.get<double>();
      }
      ...
      if (!gjkepa.is<picojson::null>()) {
        ...
        if (!gjkMaxIters.is<picojson::null>())
          PhysicsSettings::get().gjkepa.gjkMaxIters = (unsigned int)gjkMaxIters.get<double>();
        picojson::value epaMaxIters = gjkepa.get("epaMaxIters");
        if (!epaMaxIters.is<picojson::null>())
          PhysicsSettings::get().gjkepa.epaMaxIters = (unsigned int)epaMaxIters.get<double>();
      }

      picojson::value collisionResolution = settings.get("collisionResolution");
      if (!collisionResolution.is<picojson::null>()) {
        picojson::value penMaxIterations =
            collisionResolution.get("penMaxIterations");
        if (!penMaxIterations.is<picojson::null>())
          PhysicsSettings::get().collisionResolution.penMaxIterations =
              (unsigned int)penMaxIterations.get<double>();
        picojson::value velMaxIterations =
            collisionResolution.get("velMaxIterations");
        if (!velMaxIterations.is<picojson::null>())
          PhysicsSettings::get().collisionResolution.velMaxIterations =
              (unsigned int)velMaxIterations.get<double>();
        picojson::value minVelocityForRestitution =
            collisionResolution.get("minVelocityForRestitution");
        if (!minVelocityForRestitution.is<picojson::null>())
          PhysicsSettings::get().collisionResolution.minVelocityForRestitution =
              (float)minVelocityForRestitution.get<double>();
        picojson::value angularMovementLimitFactor =
            collisionResolution.get("angularMovementLimitFactor");
        if (!angularMovementLimitFactor.is<picojson::null>())
          PhysicsSettings::get()
              .collisionResolution.angularMovementLimitFactor =
              (float)angularMovementLimitFactor.get<double>();
        picojson::value persistentContactDistanceThreshold =
            collisionResolution.get("persistentContactDistanceThreshold");
        if (!persistentContactDistanceThreshold.is<picojson::null>())
          PhysicsSettings::get()
              .collisionResolution.persistentContactDistanceThreshold =
              (float)persistentContactDistanceThreshold.get<double>();
        picojson::value coefInterpAlpha =
            collisionResolution.get("coefInterpAlpha");
        if (!coefInterpAlpha.is<picojson::null>())
          PhysicsSettings::get().collisionResolution.coefInterpAlpha =
              (float)coefInterpAlpha.get<double>();
      }
      ...
      if (!rigidBodies.is<picojson::null>()) {
        ...
        if (!defaultMass.is<picojson::null>())
          PhysicsSettings::get().rigidBodies.defaultMass =
              (float)defaultMass.get<double>();
        picojson::value defaultRestitutionCoef =
            rigidBodies.get("defaultRestitutionCoef");
        if (!defaultRestitutionCoef.is<picojson::null>())
          PhysicsSettings::get().rigidBodies.defaultRestitutionCoef =
              (float)defaultRestitutionCoef.get<double>();
        picojson::value defaultFrictionCoef =
            rigidBodies.get("defaultFrictionCoef");
        if (!defaultFrictionCoef.is<picojson::null>())
          PhysicsSettings::get().rigidBodies.defaultFrictionCoef =
              (float)defaultFrictionCoef.get<double>();
        picojson::value sleepMotionThreshold =
            rigidBodies.get("sleepMotionThreshold");
        if (!sleepMotionThreshold.is<picojson::null>())
          PhysicsSettings::get().rigidBodies.sleepMotionThreshold =
              (float)sleepMotionThreshold.get<double>();
      }
      ...
      if (!gravity.is<picojson::null>()) {
        ...
        if (!x.is<picojson::null>())
          PhysicsSettings::get().gravity.x = (float)x.get<double>();
        picojson::value y = gravity.get("y");
        if (!y.is<picojson::null>())
          PhysicsSettings::get().gravity.y = (float)y.get<double>();
        picojson::value z = gravity.get("z");
        if (!z.is<picojson::null>())
          PhysicsSettings::get().gravity.z = (float)z.get<double>();
      }

      picojson::value timeScale = settings.get("timeScale");
      if (!timeScale.is<picojson::null>()) {
        PhysicsSettings::get().timeScale = (float)timeScale.get<double>();
      }
      ...
      if (!rendering.is<picojson::null>()) {
        ...
        if (!renderColliders.is<picojson::null>())
          PhysicsSettings::get().rendering.renderColliders =
              renderColliders.get<bool>();
        picojson::value renderContacts = rendering.get("renderContacts");
        if (!renderContacts.is<picojson::null>())
          PhysicsSettings::get().rendering.renderContacts =
              renderContacts.get<bool>();
        picojson::value renderContactNormals =
            rendering.get("renderContactNormals");
        if (!renderContactNormals.is<picojson::null>())
          PhysicsSettings::get().rendering.renderContactNormals =
              renderContactNormals.get<bool>();
        picojson::value renderSpawner = rendering.get("renderSpawner");
        if (!renderSpawner.is<picojson::null>())
          PhysicsSettings::get().rendering.renderSpawner =
              renderSpawner.get<bool>();
        picojson::value renderSelection = rendering.get("renderSelection");
        if (!renderSelection.is<picojson::null>())
          PhysicsSettings::get().rendering.renderSelection =
              renderSelection.get<bool>();
      }
    }
    ...
    if (!objects.is<picojson::null>()) {
      for (const auto o : objects.get<picojson::array>()) {
        ...
        if (!position.is<picojson::null>()) {
          ...
          if (!x.is<picojson::null>())
            objPosition.x = (float)x.get<double>();
          picojson::value y = position.get("y");
          if (!y.is<picojson::null>())
            objPosition.y = (float)y.get<double>();
          picojson::value z = position.get("z");
          if (!z.is<picojson::null>())
            objPosition.z = (float)z.get<double>();
        }
        ...
        if (!scale.is<picojson::null>()) {
          ...
          if (!x.is<picojson::null>())
            objScale.x = (float)x.get<double>();
          picojson::value y = scale.get("y");
          if (!y.is<picojson::null>())
            objScale.y = (float)y.get<double>();
          picojson::value z = scale.get("z");
          if (!z.is<picojson::null>())
            objScale.z = (float)z.get<double>();
        }
        ...
        if (!scale.is<picojson::null>()) {
          ...
          if (!x.is<picojson::null>())
            objOrientationEuler.x = (float)x.get<double>();
          picojson::value y = orientation.get("yaw");
          if (!y.is<picojson::null>())
            objOrientationEuler.y = (float)y.get<double>();
          picojson::value z = orientation.get("roll");
          if (!z.is<picojson::null>())
            objOrientationEuler.z = (float)z.get<double>();
        }
        picojson::value frictionCoef = o.get("frictionCoef");
        float objFrictionCoef =
            PhysicsSettings::get().rigidBodies.defaultFrictionCoef;
        if (!frictionCoef.is<picojson::null>())
          objFrictionCoef = (float)frictionCoef.get<double>();
        picojson::value restitutionCoef = o.get("restitutionCoef");
        float objRestitutionCoef =
            PhysicsSettings::get().rigidBodies.defaultRestitutionCoef;
        if (!restitutionCoef.is<picojson::null>())
          objRestitutionCoef = (float)restitutionCoef.get<double>();
        picojson::value mass = o.get("mass");
        float objMass = PhysicsSettings::get().rigidBodies.defaultMass;
        if (!mass.is<picojson::null>())
          objMass = (float)mass.get<double>();

        PhysicsObject* newObject = nullptr;
        if (!objType.compare("wall")) {
          newObject = spawnWallStatic(objPosition, objScale,
                                      glm::quat(RADIANS(objOrientationEuler)));
        } else if (!objType.compare("box")) {
          newObject = spawnBoxStatic(objPosition, objScale, objMass,
                                     glm::quat(RADIANS(objOrientationEuler)));
        } else if (!objType.compare("longBox")) {
          newObject =
              spawnLongBoxStatic(objPosition, objScale, objMass,
                                 glm::quat(RADIANS(objOrientationEuler)));
        } else if (!objType.compare("sphere")) {
          newObject =
              spawnSphereStatic(objPosition, objScale, objMass,
                                glm::quat(RADIANS(objOrientationEuler)));
        } else if (!objType.compare("cylinder")) {
          newObject =
              spawnCylinderStatic(objPosition, objScale, objMass,
                                  glm::quat(RADIANS(objOrientationEuler)));
        } else if (!objType.compare("capsule")) {
          newObject =
              spawnCapsuleStatic(objPosition, objScale, objMass,
                                 glm::quat(RADIANS(objOrientationEuler)));
        } else {
          continue;
        }
...
void ObjectSpawner::saveToFile(const std::string& filename) {
  ...
  /* write settings */
  {
    ...
    damping["linear"] =
        picojson::value((double)PhysicsSettings::get().damping.linear);
    damping["angular"] =
        picojson::value((double)PhysicsSettings::get().damping.angular);
    settings["damping"] = picojson::value(damping);

    picojson::object forceMultipliers;
    forceMultipliers["torque"] =
        picojson::value((double)PhysicsSettings::get().forceMultipliers.torque);
    forceMultipliers["force"] =
        picojson::value((double)PhysicsSettings::get().forceMultipliers.force);
    forceMultipliers["impulse"] = picojson::value(
        (double)PhysicsSettings::get().forceMultipliers.impulse);
    settings["forceMultipliers"] = picojson::value(forceMultipliers);

    picojson::object gjkepa;
    damping["gjkMaxIters"] =
        picojson::value((double)PhysicsSettings::get().gjkepa.gjkMaxIters);
    damping["epaMaxIters"] =
        picojson::value((double)PhysicsSettings::get().gjkepa.epaMaxIters);
    settings["gjkepa"] = picojson::value(gjkepa);

    picojson::object collisionResolution;
    collisionResolution["penMaxIterations"] = picojson::value(
        (double)PhysicsSettings::get().collisionResolution.penMaxIterations);
    collisionResolution["velMaxIterations"] = picojson::value(
        (double)PhysicsSettings::get().collisionResolution.velMaxIterations);
    collisionResolution["minVelocityForRestitution"] =
        picojson::value((double)PhysicsSettings::get()
                            .collisionResolution.minVelocityForRestitution);
    collisionResolution["angularMovementLimitFactor"] =
        picojson::value((double)PhysicsSettings::get()
                            .collisionResolution.angularMovementLimitFactor);
    collisionResolution["persistentContactDistanceThreshold"] = picojson::value(
        (double)PhysicsSettings::get()
            .collisionResolution.persistentContactDistanceThreshold);
    collisionResolution["coefInterpAlpha"] = picojson::value(
        (double)PhysicsSettings::get().collisionResolution.coefInterpAlpha);
    settings["collisionResolution"] = picojson::value(collisionResolution);

    picojson::object rigidBodies;
    rigidBodies["defaultMass"] =
        picojson::value((double)PhysicsSettings::get().rigidBodies.defaultMass);
    rigidBodies["defaultRestitutionCoef"] = picojson::value(
        (double)PhysicsSettings::get().rigidBodies.defaultRestitutionCoef);
    rigidBodies["defaultFrictionCoef"] = picojson::value(
        (double)PhysicsSettings::get().rigidBodies.defaultFrictionCoef);
    rigidBodies["sleepMotionThreshold"] = picojson::value(
        (double)PhysicsSettings::get().rigidBodies.sleepMotionThreshold);
    ...
    settings["timeScale"] =
        picojson::value((double)PhysicsSettings::get().timeScale);

    picojson::object rendering;
    rendering["renderColliders"] =
        picojson::value(PhysicsSettings::get().rendering.renderColliders);
    rendering["renderContacts"] =
        picojson::value(PhysicsSettings::get().rendering.renderContacts);
    rendering["renderContactNormals"] =
        picojson::value(PhysicsSettings::get().rendering.renderContactNormals);
    rendering["renderSpawner"] =
        picojson::value(PhysicsSettings::get().rendering.renderSpawner);
    rendering["renderSelection"] =
        picojson::value(PhysicsSettings::get().rendering.renderSelection);
  ...
  /* write objects */
  {
    ...
    for (auto o : this->objects) {
      ...
      glm::vec3 euler =
          DEGREES(glm::eulerAngles(glm::normalize(o->body->orientation)));
      ...
      object["restitutionCoef"] =
          picojson::value((double)o->body->restitutionCoef);
...
PhysicsObject* ObjectSpawner::spawnBoxDynamic() {
  ...
  cube->setMass(mass);
  cube->setInertiaTensor(RigidBody::inertiaTensorCube(
      PhysicsSettings::get().shapes.box.halfSizes * cube->scale));
  ...
  if (randomizeProperties)
    cube->applyTorqueAtLocalPoint(spawnDirection * 10.0f,
                                  glm::sphericalRand(2.0f));
  PhysicsObject* obj = new PhysicsObject();
  obj->body = cube;
  obj->mesh = (*meshes)["box"];
  obj->color = color;
  obj->collider = new OBBCollider(
      PhysicsSettings::get().shapes.box.halfSizes * obj->body->scale, obj,
      (*meshes)["box"]);
  ...
  obj->type.assign("box");
  obj->shape =
      new Box(PhysicsSettings::get().shapes.box.halfSizes * obj->body->scale);
...
PhysicsObject* ObjectSpawner::spawnBoxStatic(glm::vec3 position,
                                             glm::vec3 scale,
                                             float mass,
  ...
  box->setMass(mass);
  box->setInertiaTensor(RigidBody::inertiaTensorCube(
      PhysicsSettings::get().shapes.box.halfSizes * box->scale));
...
PhysicsObject* ObjectSpawner::spawnLongBoxDynamic() {
  ...
  longBox->setInertiaTensor(RigidBody::inertiaTensorCube(
      PhysicsSettings::get().shapes.box.halfSizes * longBox->scale));
  ...
  if (randomizeProperties)
    longBox->applyTorqueAtLocalPoint(spawnDirection * 10.0f,
                                     glm::sphericalRand(2.0f));
  PhysicsObject* obj = new PhysicsObject();
  obj->body = longBox;
  obj->mesh = (*meshes)["box"];
...
PhysicsObject* ObjectSpawner::spawnLongBoxStatic(glm::vec3 position,
                                                 glm::vec3 scale,
                                                 float mass,
                                                 glm::quat orientation) {
...
PhysicsObject* ObjectSpawner::spawnWallStatic(glm::vec3 position,
                                              glm::vec3 scale,
                                              glm::quat orientation) {
  ...
  wall->setInertiaTensor(RigidBody::inertiaTensorCube(
      PhysicsSettings::get().shapes.box.halfSizes * wall->scale));
...
PhysicsObject* ObjectSpawner::spawnSphereDynamic() {
  ...
  sphere->setMass(mass);
  sphere->setInertiaTensor(RigidBody::inertiaTensorSphere(
      PhysicsSettings::get().shapes.sphere.radius * sphere->scale.x));
  ...
  if (randomizeProperties)
    sphere->applyTorqueAtLocalPoint(spawnDirection * 10.0f,
                                    glm::sphericalRand(2.0f));
  PhysicsObject* obj = new PhysicsObject();
  obj->body = sphere;
  ...
  obj->color = color;
  obj->collider = new OBBCollider(
      PhysicsSettings::get().shapes.sphere.obbHalfSizes * obj->body->scale, obj,
      (*meshes)["box"]);
  obj->collider->setRigidBody(obj->body);
  obj->collider->updateInternals();
  ...
  obj->shape =
      new Sphere(PhysicsSettings::get().shapes.sphere.radius * sphere->scale.x);
  objects.push_back(obj);
  pcd->addCollider((OBBCollider*)obj->collider);
...
PhysicsObject* ObjectSpawner::spawnSphereStatic(glm::vec3 position,
                                                glm::vec3 scale,
                                                float mass,
                                                glm::quat orientation) {
  glm::vec3 color = glm::vec3(1, 1, 0);
...
PhysicsObject* ObjectSpawner::spawnCylinderDynamic() {
  ...
  cylinder->setInertiaTensor(RigidBody::inertiaTensorCylinder(
      PhysicsSettings::get().shapes.cylinder.height * cylinder->scale.y,
      PhysicsSettings::get().shapes.cylinder.radius * cylinder->scale.x));
  ...
  if (randomizeProperties)
    cylinder->applyTorqueAtLocalPoint(spawnDirection * 10.0f,
                                      glm::sphericalRand(2.0f));
  PhysicsObject* obj = new PhysicsObject();
  obj->body = cylinder;
...
PhysicsObject* ObjectSpawner::spawnCylinderStatic(glm::vec3 position,
                                                  glm::vec3 scale,
                                                  float mass,
                                                  glm::quat orientation) {
  glm::vec3 color = glm::vec3(1, 1, 0);
...
PhysicsObject* ObjectSpawner::spawnCapsuleDynamic() {
  ...
  capsule->setInertiaTensor(RigidBody::inertiaTensorCapsule(
      PhysicsSettings::get().shapes.capsule.height * capsule->scale.y,
      PhysicsSettings::get().shapes.capsule.radius * capsule->scale.x));
  ...
  if (randomizeProperties)
    capsule->applyTorqueAtLocalPoint(spawnDirection * 10.0f,
                                     glm::sphericalRand(2.0f));
  PhysicsObject* obj = new PhysicsObject();
  obj->body = capsule;
  ...
  obj->color = color;
  obj->collider = new OBBCollider(
      PhysicsSettings::get().shapes.capsule.obbHalfSizes * obj->body->scale,
      obj, (*meshes)["box"]);
  obj->collider->setRigidBody(obj->body);
  obj->collider->updateInternals();
...
PhysicsObject* ObjectSpawner::spawnCapsuleStatic(glm::vec3 position,
                                                 glm::vec3 scale,
                                                 float mass,
                                                 glm::quat orientation) {
  glm::vec3 color = glm::vec3(1, 1, 0);
