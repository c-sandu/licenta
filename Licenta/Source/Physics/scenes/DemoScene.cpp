...
void DemoScene::Init() {
  ...
  /* Initialize shaders */
  {
    ...
    shader->AddShader("Source/Physics/shaders/VertexShader.glsl",
                      GL_VERTEX_SHADER);
    shader->AddShader("Source/Physics/shaders/FragmentShader.glsl",
                      GL_FRAGMENT_SHADER);
  ...
  {
    lightPosition =
        PhysicsSettings::get().sceneProperties.lightning.lightPosition;
    lightDirection =
        PhysicsSettings::get().sceneProperties.lightning.lightDirection;
    materialShininess =
        PhysicsSettings::get().sceneProperties.lightning.materialShininess;
  ...
  GetSceneCamera()->SetPosition(
      PhysicsSettings::get().sceneProperties.defaultCameraPosition);
...
void DemoScene::Update(float deltaTime) {
  deltaTime *= PhysicsSettings::get().timeScale;
  ...
  for (auto& obj : objSpawner->objects) {
    if (obj == objSpawner->selectedObject &&
        PhysicsSettings::get().rendering.renderSelection) {
      materialKd = 0.9f;
    }
    glPolygonMode(GL_FRONT_AND_BACK, polygonMode);
    RenderSimpleMesh(obj->mesh, shaders["phong"], obj->getTransformMatrix(),
                     obj->color);
    materialKd = PhysicsSettings::get().sceneProperties.lightning.materialKd;
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    if (PhysicsSettings::get().rendering.renderColliders) {
      RenderSimpleMesh(obj->collider->mesh, shaders["phong"],
                       obj->getColTransformMatrix(), glm::vec3(1) - obj->color);
      glPolygonMode(GL_FRONT_AND_BACK, polygonMode);
    }
  }
  ...
  if (PhysicsSettings::get().rendering.renderContacts) {
    for (auto manIt : icr.manifolds) {
      for (auto cIt : manIt.contacts) {
        glm::mat4 t = glm::scale(glm::translate(glm::mat4(1), cIt->points[0]),
                                 glm::vec3(0.25f));
        RenderSimpleMesh(meshes["sphere"], shaders["phong"], t,
                         glm::vec3(1, 0, 0));

        if (PhysicsSettings::get().rendering.renderContactNormals) {
          glm::mat4 t = glm::scale(
              glm::translate(glm::mat4(1), cIt->points[0]) *
                  glm::rotate(glm::mat4(cIt->matContactToWorld), RADIANS(-90),
                              glm::vec3(0, 0, 1)) *
                  glm::translate(glm::mat4(1), glm::vec3(0, -0.5, 0)),
              glm::vec3(0.1f, 0.25f, 0.1f));
          RenderSimpleMesh(meshes["cone"], shaders["phong"], t,
                           glm::vec3(0, 1, 0));
        }
      }
    }
  ...
  if (PhysicsSettings::get().rendering.renderSpawner) {
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glm::mat4 t =
        glm::scale(glm::translate(glm::mat4(1), objSpawner->spawnPosition),
                   glm::vec3(0.01f));
  ...
  objSpawner->updateFromCamera(
      GetSceneCamera()->transform->GetWorldPosition(),
      glm::normalize(glm::vec3(-glm::row(GetSceneCamera()->View, 2))));
...
void DemoScene::RenderSimpleMesh(Mesh* mesh,
                                 Shader* shader,
...
void DemoScene::OnInputUpdate(float deltaTime, int mods) {
  deltaTime *= PhysicsSettings::get().timeScale;
  if (!window->MouseHold(GLFW_MOUSE_BUTTON_2)) {
    if (!objSpawner->selectedObject->body->isStatic) {
      if (window->KeyHold(GLFW_KEY_W)) {
        objSpawner->selectedObject->body->position +=
            glm::vec3(0, 0, -1) * deltaTime;
      ...
      if (window->KeyHold(GLFW_KEY_SEMICOLON)) {
        objSpawner->selectedObject->body->orientation = glm::normalize(
            glm::rotate(objSpawner->selectedObject->body->orientation,
                        1.0f * deltaTime, glm::vec3(0, 1, 0)));
        objSpawner->selectedObject->body->isAwake = true;
        objSpawner->selectedObject->body->resetMovement();
      }
...
void DemoScene::OnKeyPress(int key, int mods) {
  /* add key press event */
  if (key == GLFW_KEY_P) {
    switch (polygonMode) {
      case GL_POINT:
...
void DemoScene::OnKeyRelease(int key, int mods) {
  if (key == GLFW_KEY_DELETE)
...
void DemoScene::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY) {}

void DemoScene::OnMouseBtnPress(int mouseX, int mouseY, int button, int mods) {
  ...
  if (IS_BIT_SET(button, GLFW_MOUSE_BUTTON_LEFT)) {
    ...
    glm::vec4 ray_eye = glm::inverse(GetSceneCamera()->GetProjectionMatrix()) *
                        glm::vec4(x, y, -1.0f, 1.0f);
    ray_eye = glm::vec4(ray_eye.x, ray_eye.y, -1.0f, 0.0f);

    /* world coordinates */
    glm::vec3 ray_world =
        glm::inverse(GetSceneCamera()->GetViewMatrix()) * ray_eye;
    ray_world = glm::normalize(ray_world);

    PhysicsObject* newSelectedObj = pcd.performRayIntersection(
        GetSceneCamera()->transform->GetWorldPosition(), ray_world);
    if (newSelectedObj != nullptr)
      objSpawner->selectedObject = newSelectedObj;
  }
}

void DemoScene::OnMouseBtnRelease(int mouseX,
                                  int mouseY,
                                  int button,
                                  int mods) {}

void DemoScene::OnMouseScroll(int mouseX,
                              int mouseY,
                              int offsetX,
                              int offsetY) {
...
void DemoScene::OnWindowResize(int width, int height) {}
