...
class PhysicsObject {
 public:
  ...
  Collider*
      collider; /* object's collider used in broad phase collision detection */

  std::string type;

  void update(float deltaTime); /* performs one frame's worth of updating the
                                   object state */
  glm::mat4 getTransformMatrix()
      const; /* returns the local to world transform matrix of the object */
  glm::mat4 getColTransformMatrix(); /* returns the local to world transform
                                        matrix of the object's colldier */
...
class ObjectSpawner {
 public:
  ...
  ObjectSpawner(std::unordered_map<std::string, Mesh*>* meshes,
                PotentialCollisionDetector* pcd);
  ...
  PhysicsObject* spawnBoxStatic(
      glm::vec3 position,
      glm::vec3 scale = glm::vec3(1),
      float mass = PhysicsSettings::get().rigidBodies.defaultMass,
      glm::quat orientation = glm::quat(1, 0, 0, 0));
  PhysicsObject* spawnLongBoxDynamic();
  PhysicsObject* spawnLongBoxStatic(
      glm::vec3 position,
      glm::vec3 scale = glm::vec3(1),
      float mass = PhysicsSettings::get().rigidBodies.defaultMass,
      glm::quat orientation = glm::quat(1, 0, 0, 0));
  PhysicsObject* spawnWallStatic(glm::vec3 position,
                                 glm::vec3 scale = glm::vec3(1),
                                 glm::quat orientation = glm::quat(1, 0, 0, 0));
  PhysicsObject* spawnSphereDynamic();
  PhysicsObject* spawnSphereStatic(
      glm::vec3 position,
      glm::vec3 scale = glm::vec3(1),
      float mass = PhysicsSettings::get().rigidBodies.defaultMass,
      glm::quat orientation = glm::quat(1, 0, 0, 0));
  PhysicsObject* spawnCylinderDynamic();
  PhysicsObject* spawnCylinderStatic(
      glm::vec3 position,
      glm::vec3 scale = glm::vec3(1),
      float mass = PhysicsSettings::get().rigidBodies.defaultMass,
      glm::quat orientation = glm::quat(1, 0, 0, 0));
  PhysicsObject* spawnCapsuleDynamic();
  PhysicsObject* spawnCapsuleStatic(
      glm::vec3 position,
      glm::vec3 scale = glm::vec3(1),
      float mass = PhysicsSettings::get().rigidBodies.defaultMass,
      glm::quat orientation = glm::quat(1, 0, 0, 0));
