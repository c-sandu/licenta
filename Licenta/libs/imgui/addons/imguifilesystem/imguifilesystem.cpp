...
//- Common Code For All Addons needed just to ease inclusion as separate files
// in user code ----------------------
...
#if (!defined(IMGUIFS_MEMORY_USES_CHARS_AS_BYTES) || \
     !defined(IMGUIFILESYSTEM_USE_ASCII_SHORT_PATHS_ON_WINDOWS))
const int MAX_FILENAME_BYTES =
    FILENAME_MAX * 4;  // Worst case: 4 bytes per char, but huge waste of memory
                       // [we SHOULD have used imguistring.h!]
...
/*
    // Never tested (I've just been told that cl.exe does not have strcasecmp:
   please search the web for other possible alternative implementations) inline
   static int strcasecmp( const char *s1, const char *s2 )  { return
   _stricmp(s1,s2);
...
// Definitions of some helper classes (String,Path,SortingHelper,Directory).
// Better not expose them in the header file----------
/*
...
// LIMITATIONS:
// -> paths with '/','\\','.' bytes (and possibly a few others) inside multibyte
codepoints are not supported       (*)
// -> file extensions composed by characters with more than one byte are not
supported                              (**)

//(*) That's because when I call something like: mystring.find_last_of('/') or
Path::Split('/')
// the result might not be correct if there's some multibyte codepoint that
includes that byte(s) (bytes include '/','\\','.').

// (**) String::ToLower() deeply breaks any multibyte char.
// They're currently used only in:
// Path::GetExtension(...)
// Directory::GetFiles(const string& path,const string& wantedExtensions,const
string& unwantedExtensions)
// That's because file extensions must be returned lowercase, so that e.g.
".PNG" and ".png" can be string matched (even on case sensitive file systems).
*/
class String {
 protected:
  String() {}
 ...
 public:
  inline static void PushBack(FilenameStringVector& rv, const char* s) {
    if (rv.Size == rv.Capacity)
      rv.reserve(rv._grow_capacity(
          rv.Size + 1));  // optional optimization from ImVector<>::push_back()
    const size_t sz = rv.size();
    rv.resize(sz + 1);
    strcpy(&rv[sz][0], s ? s : "\0");
  }
#if (FILENAME_MAX != \
     DIRENT_MAX_PATH)  // Will this work ? (I don't want to use templates)
  ...
  inline static void Substr(const char* text,
                            char* rv,
                            int start,
                            int count = -1) {
    if (!text)
  ...
  inline static void Split(const char* text,
  ...
  inline static void Replace(const char* baseText,
  ...
  inline static void ReplaceInPlace(char* text,
  ...
  inline static void wide_to_utf8(const wchar_t* wstr, char* rv) {
    ...
    // rv[size_needed]='\0';              // If the parameter after wstr is -1,
    // the function processes the entire input string, including the terminating
    // null character. Therefore, the resulting character string has a
    // terminating null character, and the length returned by the function
  ...
  inline static void utf8_to_wide(const char* str, wchar_t* rv) {
    ...
    // rv[size_needed]=L'\0';            // // If the parameter after str is -1,
    // the function processes the entire input string, including the terminating
    // null character. Therefore, the resulting character string has a
    // terminating null character, and the length returned by the function
    // includes this character.
...
class Path {
 ...
 public:
  static void GetAbsolutePath(const char* path, char* rv) {
    ...
    // strlen(path)); // TO remove!
    static const int bufferSize =
        DIRENT_MAX_PATH + 1;  // 4097 is good (PATH_MAX should be in <limits.h>,
  ...
  static void ChangeExtension(const char* filePath,
  ...
  static void Combine(const char* directory,
                      const char* fileName,
                      char* rv,
  ...
  static void Split(const char* path,
                    FilenameStringVector& rv,
    ...
    while ((beg = String::Find(tex, '/')) != -1) {
      ...
      String::Substr(tex, tmp, 0,
                     leaveIntermediateTrailingSlashes ? beg + 1 : beg);
...
class SortingHelper {
 public:
  typedef int (*SorterSignature)(const struct dirent** e1,
 ...
 protected:
  ...
  static int IMGUIFS_CDECL Alphasort(const struct dirent** e1,
                                     const struct dirent** e2) {
    return strcasecmp((*e1)->d_name, (*e2)->d_name);
  }
  static int IMGUIFS_CDECL Alphasortinverse(const struct dirent** e1,
                                            const struct dirent** e2) {
    return -strcasecmp((*e1)->d_name, (*e2)->d_name);
  }
  // Please note that calling stat(...) every time inside sorters is a suicide!
  // And that I'm doing right that! (but I guess and hope that on many systems
  // the calls get cached somewhere: otherwise it would take ages to sort)
  static int IMGUIFS_CDECL Lastmodsort(const struct dirent** e1,
                                       const struct dirent** e2) {
    ...
    return (stat1.st_mtime < stat2.st_mtime
                ? -1
                : stat1.st_mtime > stat2.st_mtime ? 1 : 0);
  }
  static int IMGUIFS_CDECL Lastmodsortinverse(const struct dirent** e1,
                                              const struct dirent** e2) {
    if (stat((*e1)->d_name, &stat1) == -1)
      return 1;
    if (stat((*e2)->d_name, &stat2) == -1)
    ...
    return (stat2.st_mtime < stat1.st_mtime
                ? -1
                : stat2.st_mtime > stat1.st_mtime ? 1 : 0);
  }
  static int IMGUIFS_CDECL Sizesort(const struct dirent** e1,
                                    const struct dirent** e2) {
    if (stat((*e1)->d_name, &stat1) == -1)
      return -1;
    if (stat((*e2)->d_name, &stat2) == -1)
    ...
    return (stat1.st_size < stat2.st_size
                ? -1
                : stat1.st_size > stat2.st_size ? 1 : 0);
  }
  static int IMGUIFS_CDECL Sizesortinverse(const struct dirent** e1,
                                           const struct dirent** e2) {
    if (stat((*e1)->d_name, &stat1) == -1)
      return 1;
    if (stat((*e2)->d_name, &stat2) == -1)
    ...
    return (stat2.st_size < stat1.st_size
                ? -1
                : stat2.st_size > stat1.st_size ? 1 : 0);
  }
  // Please note that calculating the file extension every time inside sorters
  // is a suicide (well, much less than before...)!
  static int IMGUIFS_CDECL Typesort(const struct dirent** e1,
                                    const struct dirent** e2) {
    static const int dot = (int)'.';
  ...
  static int IMGUIFS_CDECL Typesortinverse(const struct dirent** e1,
                                           const struct dirent** e2) {
    static const int dot = (int)'.';
    const char* p1 = strrchr((const char*)(*e1)->d_name, dot);
    const char* p2 = strrchr((const char*)(*e2)->d_name, dot);
...
const SortingHelper::SorterSignature SortingHelper::Sorters[] = {
    &SortingHelper::Alphasort,   &SortingHelper::Alphasortinverse,
    &SortingHelper::Lastmodsort, &SortingHelper::Lastmodsortinverse,
    &SortingHelper::Sizesort,    &SortingHelper::Sizesortinverse,
...
class Directory {
 public:
  static void GetDirectories(const char* directoryName,
...
#endif  //_WIN32
    const int n = scandir(directoryName2, &eps, DirentGetDirectories,
                          SortingHelper::SetSorter(sorting));
      ...
      for (int cnt = 0; cnt < n; ++cnt) {
        ...
        if (sz > 0) {
          if (strcmp(pName, ".") != 0 && strcmp(pName, "..") != 0 &&
              ...
              && (!(strcmp(pName, "fd") == 0 &&
                    strcmp(directoryNameWithoutSlash, "/proc/self") == 0))
  ...
  static void GetFiles(const char* directoryName,
                       PathStringVector& result,
                       FilenameStringVector* pOptionalNamesOut = NULL,
                       Sorting sorting = SORT_ORDER_ALPHABETIC) {
    result.clear();
...
#endif  //_WIN32
    const int n = scandir(directoryName2, &eps, DirentGetFiles,
                          SortingHelper::SetSorter(sorting));
  ...
  static void GetFiles(const char* path,
...
#ifndef _WIN32
    const mode_t mode =
        S_IFDIR | S_IREAD | S_IWRITE | S_IRWXU | S_IRWXG | S_IRWXO;
  ...
  inline static const PathStringVector& GetUserKnownDirectories(
    ...
    if (pOptionalNumberKnownUserDirectoriesExceptDrives)
      *pOptionalNumberKnownUserDirectoriesExceptDrives =
          &numberKnownUserDirectoriesExceptDrives;
    if (init && !forceUpdate)
      return rv;
    init = true;
    rv.clear();
    ...
    static const int csid[] = {CSIDL_DESKTOP,
                               ...
                               CSIDL_FAVORITES, CSIDL_MYMUSIC, CSIDL_MYPICTURES,
                               CSIDL_RECENT,    CSIDL_MYVIDEO};
    static const char* name[] = {"Desktop",  "Documents", "Favorites", "Music",
                                 "Pictures", "Recent",    "Video"};
    ...
    if (csidSize != nameSize)
      fprintf(stderr,
              "ERROR in file: imguifilesystem.cpp. "
              "Directory::GetUserKnownDirectories(...) csidSize!=nameSize.\n");
    ...
    numberKnownUserDirectoriesExceptDrives = (int)rv.size();
    // Additional Drives --------------------------------------------
    static const char* mountLocations[] = {"/media", "/mnt", "/Volumes", "/vol",
                                           "/data"};
    static const int mountLocationSize =
        sizeof(mountLocations) / sizeof(mountLocations[0]);
    static const bool
        ifHomeSubfolerIsFoundInMountLocationsForgetThatRootMountLocation =
            true;  // That means: if "/media/myusername" exists, don't add any
                   // other "/media/..." entries.
    char userMediaString[MAX_PATH_BYTES];
    bool lastGood = false;
    for (int mntLocIndex = 0, sz = 2 * mountLocationSize; mntLocIndex < sz;
         mntLocIndex++) {
      ...
      for (int i = 0, msz = mediaFolders.size(); i < msz; i++) {
        if (strcmp(mediaFolders[i], temp) == 0)
          continue;  // I we have processed "/media/myusername" once cycle
                     // before, exclude it from processing "/media" subfolders
...
#ifdef _WIN32
  static bool GetSpecialFolderPathW(int specialFolderCSIDL,
                                    WCHAR* pathOutWithSizeMaxPathPlusOne,
                                    HWND parent) {
    ...
    if (!SUCCEEDED(
            ::SHGetSpecialFolderLocation(parent, specialFolderCSIDL, &pidl))) {
      temp_path[0] = L'\0';
      return false;
    }
...
struct UnZipFileImpl {
  ...
  struct unz_file_info64_plus {
    ...
    static void Sort(ImVector<unz_file_info64_plus>& fileInfos,
                     Sorting sorting) {
      if (fileInfos.size() == 0)
        return;
      ...
      if (sorter)
        qsort(&fileInfos[0], fileInfos.size(), sizeof(unz_file_info64_plus),
              sorter);
    }
    // void qsort( void *ptr, size_t count, size_t size,int (*comp)(const void
    // *, const void *) );
    ...
    inline static int IMGUIFS_CDECL Alphasortinverse(const void* e1,
                                                     const void* e2) {
      return -strcasecmp(static_cast<const unz_file_info64_plus*>(e1)->name,
                         static_cast<const unz_file_info64_plus*>(e2)->name);
    }
    inline static int IMGUIFS_CDECL
    LastModsort(const void* e1, const void* e2) {  // or tmu_date ?
      const uLong& dosDate1 =
          static_cast<const unz_file_info64_plus*>(e1)->info.dosDate;
      const uLong& dosDate2 =
          static_cast<const unz_file_info64_plus*>(e2)->info.dosDate;
      return (dosDate1 < dosDate2) ? -1 : (dosDate1 > dosDate2) ? 1 : 0;
    }
    inline static int IMGUIFS_CDECL LastModsortinverse(const void* e1,
                                                       const void* e2) {
      const uLong& dosDate1 =
          static_cast<const unz_file_info64_plus*>(e1)->info.dosDate;
      const uLong& dosDate2 =
    ...
    inline static int IMGUIFS_CDECL Sizesortinverse(const void* e1,
                                                    const void* e2) {
      const ZPOS64_T& size1 =
          static_cast<const unz_file_info64_plus*>(e1)->info.uncompressed_size;
      const ZPOS64_T& size2 =
    ...
    inline static int IMGUIFS_CDECL Typesort(const void* e1, const void* e2) {
      static const int dot = (int)'.';
      const char* p1 = strrchr(
          (const char*)static_cast<const unz_file_info64_plus*>(e1)->name, dot);
      const char* p2 = strrchr(
          (const char*)static_cast<const unz_file_info64_plus*>(e2)->name, dot);
      if (!p1)
        return (!p2 ? 0 : -1);
      else if (!p2)
        return 1;
    ...
    inline static int IMGUIFS_CDECL Typesortinverse(const void* e1,
                                                    const void* e2) {
      static const int dot = (int)'.';
      const char* p1 = strrchr(
          (const char*)static_cast<const unz_file_info64_plus*>(e1)->name, dot);
  ...
  bool getFilesOrDirectories(
    ...
    if (directoryName) {
      ...
      if (dirNameLen > 1 && directoryName[0] == '.' &&
    ...
    while (dirNameLen > 0 && (dirName[dirNameLen - 1] == '/' ||
    ...
    for (i = 0; i < gi.number_entry; i++) {
      ...
      ok = fileMode ? (!isDirectory && !hasZeroSize)
                    : (isDirectory && hasZeroSize) ? true : false;
      ...
      if (ok) {
        ...
        if (ok) {
          ...
          strcpy(tmp, dirNameLen > 0 ? &filename_inzip[dirNameLen + 1]
                                     : filename_inzip);  // Here "tmp" if the
                                                         // plain name, and
                                                         // "filename_inzip" the
                                                         // full name
          ...
          if (ok) {
            if ((file_info.flag & 1) != 0)
              strcat(tmp, charCrypt);  // Optional display a '*' in the file
                                       // name only if it is crypted
  ...
  static bool PathSplitFirstZipFolder(const char* path,
                                      char* rv1,
                                      char* rv2,
                                      bool rv1IsAbsolutePath = true) {
    rv1[0] = '\0';
    rv2[0] = '\0';
    if (path) {
      int gc = 0;
  ...
  static bool PathExistsWithZipSupport(const char* path,
...
bool UnZipFile::load(const char* zipFilePath, bool reloadIfAlreadyLoaded) {
  if (!reloadIfAlreadyLoaded && isValid() &&
      strcmp(zipFilePath, im->zipFilePath) == 0)
    return true;
  close();
  if (zipFilePath) {
...
bool UnZipFile::getDirectories(
    ...
    Sorting sorting,
    bool prefixResultWithTheFullPathOfTheZipFile) const {
  return im->getFilesOrDirectories(false, directoryName, result,
                                   pOptionalNamesOut, sorting,
                                   prefixResultWithTheFullPathOfTheZipFile);
}
bool UnZipFile::getFiles(const char* directoryName,
                         PathStringVector& result,
                         FilenameStringVector* pOptionalNamesOut,
                         Sorting sorting,
                         bool prefixResultWithTheFullPathOfTheZipFile) const {
  return im->getFilesOrDirectories(true, directoryName, result,
                                   pOptionalNamesOut, sorting,
                                   prefixResultWithTheFullPathOfTheZipFile);
}
unsigned int UnZipFile::getFileSize(const char* filePath) const {
  ...
  int err = unzGetCurrentFileInfo64(im->uf, &file_info, filename_inzip,
                                    sizeof(filename_inzip), NULL, 0, NULL, 0);
  if (err != UNZ_OK)
    return 0;
...
static bool UnZipFileGetFileContentBase(ImGuiFs::UnZipFileImpl* im,
  ...
  if (err < 0) {
    fprintf(stderr,
            "Error while unzipping: \"%s\": %d with zipfile in "
            "unzGetCurrentFileInfo\n",
            filePath, err);
    return false;
  }
  // if (file_info.uncompressed_size>MAX_UNSIGNED_INT) return false; // where is
  // MAX_UNSIGNED_INT ???
...
bool UnZipFile::getFileContent(const char* filePath,
                               ImVector<unsigned char>& bufferOut,
                               const char* password) const {
  return UnZipFileGetFileContentBase<unsigned char>(im, filePath, bufferOut,
                                                    password);
}
bool UnZipFile::getFileContent(const char* filePath,
...
bool UnZipFile::exists(const char* pathInsideZip,
                       bool reportOnlyFiles,
                       bool reportOnlyDirectories) const {
  ...
  if (pathInsideZip) {
    ...
    if (dirNameLen > 1 && pathInsideZip[0] == '.' &&
  ...
  while (dirNameLen > 0 &&
         (path[dirNameLen - 1] == '/' || path[dirNameLen - 1] == '\\')) {
    path[dirNameLen - 1] = '\0';
    --dirNameLen;
  }

  bool onlyDirs = false, onlyFiles = false;
  if ((reportOnlyFiles || reportOnlyDirectories) &&
      !(reportOnlyFiles && reportOnlyDirectories)) {
  ...
  bool found = (unzLocateFile(im->uf, path, 0) ==
                UNZ_OK);  // it becames the current file
  ...
  if (found && (onlyDirs || onlyFiles)) {
    ...
    if (err != UNZ_OK) {
      ...
      found = onlyFiles ? (!isDirectory && !hasZeroSize)
                        : onlyDirs ? (isDirectory && hasZeroSize) : false;
    }
  }
  unzCloseCurrentFile(im->uf);
...
bool PathSplitFirstZipFolder(const char* path,
                             char* rv1,
                             char* rv2,
                             bool rv1IsAbsolutePath) {
  return UnZipFileImpl::PathSplitFirstZipFolder(path, rv1, rv2,
                                                rv1IsAbsolutePath);
}
bool PathExistsWithZipSupport(const char* path,
                              bool reportOnlyFiles,
                              bool reportOnlyDirectories,
                              bool checkAbsolutePath,
                              bool* isInsideAZipFile) {
  return UnZipFileImpl::PathExistsWithZipSupport(
      path, reportOnlyFiles, reportOnlyDirectories, checkAbsolutePath,
      isInsideAZipFile);
}
bool PathIsInsideAZipFile(const char* path) {
  bool isInsideAZipFile = false;
  UnZipFileImpl::PathExistsWithZipSupport(path, true, true, true,
                                          &isInsideAZipFile);
...
bool DirectoryGetDirectoriesWithZipSupport(
  ...
  if (PathSplitFirstZipFolder(directoryName, zipRoot, zipFolder)) {
    ...
    if (!uzf.isValid())
      return false;
    return uzf.getDirectories(zipFolder, result, pOptionalNamesOut, sorting,
                              prefixResultWithTheFullPathOfTheZipFile);
  }
  ImGuiFs::DirectoryGetDirectories(directoryName, result, pOptionalNamesOut,
                                   sorting);
  return true;
}
bool DirectoryGetFilesWithZipSupport(
  ...
  if (PathSplitFirstZipFolder(directoryName, zipRoot, zipFolder)) {
    ...
    if (!uzf.isValid())
      return false;
    return uzf.getFiles(zipFolder, result, pOptionalNamesOut, sorting,
                        prefixResultWithTheFullPathOfTheZipFile);
  }
...
void PathGetAbsoluteWithZipSupport(const char* path, char* rv) {
  ...
  if (PathSplitFirstZipFolder(path, zipRoot, zipRelativePath)) {
    ...
    strcpy(rv, tmp);
    if (strlen(zipRelativePath) > 0 && zipRelativePath[0] != '/' &&
        zipRelativePath[0] != '\\')
      strcat(rv, "/");
...
template <typename CharType>
bool FileGetContentBase(const char* path,
                        ImVector<CharType>& bufferOut,
                        bool openInTextMode,
                        const char* password) {
  bufferOut.clear();
...
bool FileGetContent(const char* path,
                    ImVector<unsigned char>& bufferOut,
                    bool openInTextMode,
                    const char* password) {
  return FileGetContentBase<unsigned char>(path, bufferOut, openInTextMode,
                                           password);
}
bool FileGetContent(const char* path,
                    ImVector<char>& bufferOut,
...
struct ImGuiFsDrawIconStruct {
  ...
  int add(const char* exts) {
    ...
    const int vStartersCountValue =
        vStartersCounts.size() > 0
            ? (vStartersCounts[vStartersCounts.size() - 1] + 1)
            : 0;
    static char tmp[512];
  ...
  inline bool drawIcon(int extensionType,
                       const ImVec4* pOptionalColorOverride = NULL) const {
    return ImGuiFs::Dialog::DrawFileIconCallback
               ? ImGuiFs::Dialog::DrawFileIconCallback(extensionType,
                                                       pOptionalColorOverride)
               : false;
  }
  inline bool drawIcon(const char* ext,
  ...
  // New to avoid global static initialization: see
  // https://github.com/Flix01/imgui/issues/38
...
void FileGetExtensionTypesFromFilenames(ImVector<int>& fileExtensionTypesOut,
                                        const FilenameStringVector& fileNames) {
  ImGuiFsDrawIconStruct::Get().fillExtensionTypesFromFilenames(
      fileExtensionTypesOut, fileNames);
}
...
bool FileDownload(const char* path, const char* optionalSaveFileName) {
  ...
  if (optionalSaveFileName)
    buffer.appendf("saveFileFromMemoryFSToDisk(\"%s\",\"%s\")", path,
                   optionalSaveFileName);
  else {
...
struct FolderInfo {
  ...
  inline static int GetSplitPathIndexOfZipFile(
      const FilenameStringVector& SplitPath) {
    ...
    for (int j = 0, jsz = (int)SplitPath.size(); j < jsz; j++) {
      ...
      const int startCharToMatch =
          (j == jsz - 1) ? numCharsToMath : (numCharsToMath + 1);
      ...
      while (c == lower[gc] || c == upper[gc]) {
        ...
        if (++gc == numCharsToMath) {
          ...
          // fprintf(stderr,"splitPathIndexOfZipFile=%d
          // (%s)\n",splitPathIndexOfZipFile,path);
          return splitPathIndexOfZipFile;  // Warning: I should double break
                                           // here!
        }
        c = path[i + gc];
      }
  ...
  bool isEqual(const FolderInfo& fi) const {
    return strcmp(fullFolder, fi.fullFolder) == 0 &&
           strcmp(currentFolder, fi.currentFolder) == 0;
  }
  bool isEqual(const char* path) const {
    return strcmp(fullFolder, path) == 0 && strcmp(currentFolder, path) == 0;
  }
  int getSplitPathIndexFor(
      const char* path /*,int* pOptionalSplitPathIndexOfZipFile=NULL*/) const {
  ...
  bool getFolderInfoForSplitPathIndex(int _splitPathIndex,
                                      FolderInfo& rv) const {
    Path::Split(fullFolder, SplitPath);
    ...
    ",_splitPathIndex); this->display(); fprintf(stderr,"Result:   ");
    rv.display();*/
    return true;
  }
};
  ...
  bool switchTo(const char* currentFolder) {
    ...
    if (currentInfoIndex < 0) {
      ...
      if (lastInfo.isEqual(currentFolder))
        return false;
      const int splitPathIndexInsideLastInfo =
          lastInfo.getSplitPathIndexFor(currentFolder);
      ++currentInfoIndex;
      info.resize(currentInfoIndex + 1);
      FolderInfo& fi = info[currentInfoIndex];
      if (splitPathIndexInsideLastInfo == -1) {
        fi.fromCurrentFolder(currentFolder);
        // fprintf(stderr,"switchTo 2a: %d
...
struct Internal {
...
#if FILENAME_MAX != \
    DIRENT_MAX_PATH  // otherwise PathStringVector == FilenameStringVector
  ...
  inline void calculateBrowsingDataTableSizes(
      const ImVec2& childWindowSize = ImVec2(-1, -1)) {
    int approxNumEntriesPerColumn =
        20;  //(int) (20.f / browseSectionFontScale);// tweakable
    if (childWindowSize.y > 0) {
      int numLinesThatFit =
          childWindowSize.y / ImGui::GetTextLineHeightWithSpacing();
    ...
    if (totalNumBrowsingEntries % approxNumEntriesPerColumn >
        (approxNumEntriesPerColumn / 2))
      ++numBrowsingColumns;
    int maxNumBrowsingColumns =
        (childWindowSize.x > 0) ? (childWindowSize.x / 100) : 6;
  ...
  inline static void ColorCombine(ImVec4& c,
                                  const ImVec4& r,
                                  const ImVec4& factor) {
...
Dialog::Dialog(bool noKnownDirectoriesSection,
  ...
  internal->detectKnownDirectoriesAtEveryOpening =
      detectKnownDirectoriesAtEachOpening;
...
const char* Dialog::getChosenPath() const {
  return internal->chosenPath;
}
const char* Dialog::getLastDirectory() const {
  return internal->currentFolder;
}
bool Dialog::hasUserJustCancelledDialog() const {
  return internal->userHasJustCancelledDialog;
}
...
// 90% of the functionality of the whole imguifilesystem.cpp is inside this
// single method
const char* ChooseFileMainMethod(Dialog& ist,
  ...
  bool& allowDirectoryCreation = I.allowDirectoryCreation =
      I.forbidDirectoryCreation ? false
  ...
  static const PathStringVector* pUserKnownDirectories =
      &Directory::GetUserKnownDirectories(
          &pUserKnownDirectoryDisplayNames,
          &pNumberKnownUserDirectoriesExceptDrives);
  //----------------------------------------------------------
  const ImGuiStyle& style = ImGui::GetStyle();
  ImVec4 dummyButtonColor(0.0f, 0.0f, 0.0f,
                          0.5f);  // Only the alpha is twickable from here
  ImVec4 dummyZipButtonColor(0.0f, 0.0f, 0.0f,
                             0.5f);  // Only the alpha is twickable from here
  ...
  // Fill ColorSet above and fix dummyButtonColor here
  {
    ...
    for (int i = 0, sz = (int)Internal::ImGuiCol_Dialog_Directory_Text; i <= sz;
         i++) {
      ImVec4& c = ColorSet[i];
      const ImVec4& r =
          style.Colors[i < sz ? ((int)ImGuiCol_Button + i) : ImGuiCol_Text];
    ...
    if (dummyZipButtonColor.w > 0) {
      const ImVec4& bbc =
          ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Background];
  ...
  if (I.rescan) {
    ...
    if (directory && strlen(directory) > 0)
      ...
      isInsideZipFile =
          PathSplitFirstZipFolder(I.currentFolder, basePath, zipPath);
    ...
    allowDirectoryCreation = I.forbidDirectoryCreation
                                 ? false
                                 : (isSelectFolderDialog || isSaveFileDialog);
    if (isSelectFolderDialog &&
        I.sortingMode > SORT_ORDER_LAST_MODIFICATION_INVERSE)
      I.sortingMode = 0;
    I.forceRescan = true;
    I.open = true;
    I.filter.Clear();
    ...
    // fprintf(stderr,"screenSize = %f,%f mousePos = %f,%f wndPos = %f,%f
    // wndSize =
    ...
    if (I.detectKnownDirectoriesAtEveryOpening)
      pUserKnownDirectories = &Directory::GetUserKnownDirectories(
          &pUserKnownDirectoryDisplayNames,
          &pNumberKnownUserDirectoriesExceptDrives, true);

    // fprintf(stderr,"Pos: %1.0f %1.0f Size: %1.0f
    // %1.0f\n",I.wndPos.x,I.wndPos.y,I.wndSize.x,I.wndSize.y);
  }
  if (!I.open) {
    // This case (user clicks the close button) does not seem to be triggered
    // here... No matter, I just handle it in the 3 caller methods, where it's
    // triggered.
    return rv;
  }
  ...
  if (I.forceRescan) {
    ...
    const int sortingModeForDirectories =
        (I.sortingMode <= (int)SORT_ORDER_LAST_MODIFICATION_INVERSE)
            ? I.sortingMode
            : (I.sortingMode % 2);
    Directory::GetDirectories(
        I.currentFolder, I.dirs, &I.dirNames,
        (Sorting)sortingModeForDirectories);  // this is because directories
                                              // don't return their size or
                                              // their file extensions (so if
                                              // needed we sort them
                                              // alphabetically)

    if (!isSelectFolderDialog) {
      if (!fileFilterExtensionString || strlen(fileFilterExtensionString) == 0)
        Directory::GetFiles(I.currentFolder, I.files, &I.fileNames,
                            (Sorting)I.sortingMode);
      else
        Directory::GetFiles(I.currentFolder, I.files, fileFilterExtensionString,
                            NULL, &I.fileNames, (Sorting)I.sortingMode);
      ...
      if (currentFolderNameSize == 0 ||
          currentFolderName[currentFolderNameSize - 1] == ':')
    ...
    char zipPath[MAX_PATH_BYTES];
    zipPath[0] = '\0';
    const bool isInsideZipFile =
        PathSplitFirstZipFolder(I.currentFolder, basePath, zipPath);
    if (!isInsideZipFile)
      Directory::GetDirectories(
          basePath, I.dirs, &I.dirNames,
          (Sorting)sortingModeForDirectories);  // this is because directories
                                                // don't return their size or
                                                // their file extensions (so if
                                                // needed we sort them
      ...
      if (I.unz.load(basePath, false))
        I.unz.getDirectories(zipPath, I.dirs, &I.dirNames,
                             (Sorting)sortingModeForDirectories, true);
      else
        I.unz.close();
    }
    ...
    if (!isSelectFolderDialog) {
      if (!isInsideZipFile) {
        if (!fileFilterExtensionString ||
            strlen(fileFilterExtensionString) == 0)
          Directory::GetFiles(I.currentFolder, I.files, &I.fileNames,
                              (Sorting)I.sortingMode);
        else
          Directory::GetFiles(I.currentFolder, I.files,
                              fileFilterExtensionString, NULL, &I.fileNames,
                              (Sorting)I.sortingMode);
        if (Dialog::DrawFileIconCallback)
          ImGuiFsDrawIconStruct::Get().fillExtensionTypesFromFilenames(
              I.fileExtensionTypes, I.fileNames);
        ...
        /*if (!fileFilterExtensionString ||
         * strlen(fileFilterExtensionString)==0)*/
        I.unz.getFiles(zipPath, I.files, &I.fileNames, (Sorting)I.sortingMode,
                       true);
        // else
    ...
    if (I.history.getInfoSize() > 0)
      fprintf(stderr,
              "\nHISTORY: currentFolder:\"%s\" history.canGoBack=%s "
              "history.canGoForward=%s currentHistory:\n",
  ...
  if (I.rescan) {
    ...
    const bool popupOk =
        ImGui::BeginPopupModal(I.wndTitle, &I.open, Dialog::ExtraWindowFlags);
    if (!popupOk)
      return rv;
  } else {
...
#undef DEBUG_HISTOTY
  ...
  const bool isBrowsingInsideZipFile =
      (fi && (!(fi->splitPathIndexOfZipFile < 0 ||
  ...
  bool editLocationInputTextReturnPressed = false;
  {
    ...
    ImGui::PushStyleColor(ImGuiCol_Button, I.editLocationCheckButtonPressed
                                               ? dummyButtonColor
    ...
    if (ImGui::Button("L##EditLocationCheckButton")) {
      ...
      // if (!I.editLocationCheckButtonPressed) mustValidateInputPath = true;
      // // or not ? I mean: the user wants to quit or to validate in this case
      // ?
    }
    ...
    if (I.editLocationCheckButtonPressed) {
      ...
      editLocationInputTextReturnPressed = ImGui::InputText(
          "##EditLocationInputText", I.editLocationInputText, MAX_PATH_BYTES,
    ...
    if (mustValidateInputPath) {
      ...
      while (len > 0 && (cleanEnteredPath[len - 1] == '/' ||
      ...
      if (len == 0 || strcmp(I.currentFolder, cleanEnteredPath) == 0)
        ...
        I.editLocationCheckButtonPressed =
            false;  // Optional (return to split-path buttons)
        //----------------------------------------------------------------------------------
        ...
        zipPath[0] = '\0';
        isInsideZipFile =
            PathSplitFirstZipFolder(cleanEnteredPath, basePath, zipPath, false);
        if (!isInsideZipFile) {
        ...
        for (int t = 0; t < numTabs; t++) {
          if (t == fi.splitPathIndex) {
            ...
            ImGui::PushStyleColor(
                ...
                ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Hover]);
            ImGui::PushStyleColor(
...
#ifndef _MSC_VER
          ImGui::PushID(
              &I.currentSplitPath[t]);  // old cl.exe versions don't like this
#else                                   //_MSC_VER
          ...
          if (wrapMode && sumX == 0) {
            sumX =
                ImGui::GetStyle().WindowPadding.x + ImGui::GetItemRectSize().x;
          }
          ImGui::PopID();
          if (pressed) {
  ...
  if (I.allowKnownDirectoriesSection && pUserKnownDirectories->size() > 0) {
    ...
    if (ImGui::CollapsingHeader(
      ...
      ImGui::PushStyleColor(ImGuiCol_Text,
                            ColorSet[Internal::ImGuiCol_Dialog_Directory_Text]);
      ImGui::PushStyleColor(
          ImGuiCol_Button,
          ColorSet[Internal::ImGuiCol_Dialog_Directory_Background]);
      ImGui::PushStyleColor(
          ImGuiCol_ButtonHovered,
          ColorSet[Internal::ImGuiCol_Dialog_Directory_Hover]);
      ImGui::PushStyleColor(
          ImGuiCol_ButtonActive,
          ColorSet[Internal::ImGuiCol_Dialog_Directory_Pressed]);
      // const ImVec4& iconsColor =
      // ColorSet[Internal::ImGuiCol_Dialog_Directory_Text];

      for (int i = 0, sz = (int)pUserKnownDirectories->size(); i < sz; i++) {
        const char* userKnownFolder = (*pUserKnownDirectories)[i];
        const char* userKnownFolderDisplayName =
            (*pUserKnownDirectoryDisplayNames)[i];
        ...
        if (i != sz - 1 &&
            (i >= *pNumberKnownUserDirectoriesExceptDrives || i % 7 != 6))
          ImGui::SameLine();
      }
  ...
  if (allowDirectoryCreation && !isBrowsingInsideZipFile) {
    ...
    if (ImGui::CollapsingHeader(
      ...
      ImGui::InputText("##createNewFolderName", &I.newDirectoryName[0],
                       MAX_FILENAME_BYTES);
      ImGui::SameLine();
...
#undef SIMULATING_ONLY
  ...
  ImGui::SameLine();
  {
    ...
    //-----------------------------------------------------
    {
      ...
      if (I.allowDisplayByOption && I.numBrowsingColumns > 1) {
        ...
        ImGui::SameLine();
        ImGui::PushStyleColor(ImGuiCol_Button, dummyButtonColor);
        if (ImGui::SmallButton(!Internal::BrowsingPerRow
  ...
  //-----------------------------------------------------------------------------
  {
    if (ImGui::BeginChild(
            "BrowsingFrame",
            ImVec2(0, (isSaveFileDialog || isSelectFolderDialog)
                          ? (ImGui::GetContentRegionAvail().y -
                             1.2f * ImGui::GetTextLineHeightWithSpacing() -
                             ImGui::GetStyle().WindowPadding.y)
                          : 0))) {
      I.calculateBrowsingDataTableSizes(
          ImGui::GetWindowSize());  // (Actually we could save this call every
                                    // frame, but it's difficult to detect whn
                                    // we need it)

      // ImGui::SetScrollPosHere();   // possible future ref: while drawing to
      // place the scroll bar making a certain entry visible Also we don't do
      // any maual clipping here (for long directories using a clipper might be
      // useful)
      ...
      if (I.dirs.size() > 0) {
        ImGui::PushStyleColor(
            ImGuiCol_Text, ColorSet[Internal::ImGuiCol_Dialog_Directory_Text]);
        ImGui::PushStyleColor(
            ImGuiCol_Button,
            ColorSet[Internal::ImGuiCol_Dialog_Directory_Background]);
        ImGui::PushStyleColor(
      ...
      if (!isSelectFolderDialog && I.files.size() > 0) {
        ImGui::PushStyleColor(ImGuiCol_Text,
                              ColorSet[Internal::ImGuiCol_Dialog_File_Text]);
        ImGui::PushStyleColor(
            ImGuiCol_Button,
            ColorSet[Internal::ImGuiCol_Dialog_File_Background]);
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                              ColorSet[Internal::ImGuiCol_Dialog_File_Hover]);
        ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                              ColorSet[Internal::ImGuiCol_Dialog_File_Pressed]);
        const ImVec4* pIconsColor =
            &ColorSet[Internal::ImGuiCol_Dialog_File_Text];
          ...
          if (I.filter.PassFilter(fileName)) {
            ...
            if (acceptZipFilesForBrowsing) {
              ...
              if (hasZipExtension && !isZipFile) {
                ...
                ImGui::PushStyleColor(
                    ImGuiCol_Text,
                    ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Text]);
                ImGui::PushStyleColor(
                    ImGuiCol_Button,
                    ColorSet
                        [Internal::ImGuiCol_Dialog_ZipDirectory_Background]);
                ImGui::PushStyleColor(
                    ImGuiCol_ButtonHovered,
                    ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Hover]);
                ImGui::PushStyleColor(
...
#endif  // IMGUI_USE_MINIZIP
            if (Dialog::DrawFileIconCallback &&
                Dialog::DrawFileIconCallback(I.fileExtensionTypes[i],
                                             pIconsColor))
              ImGui::SameLine();
...
#endif                   // IMGUI_USE_MINIZIP
#elif IMGUI_USE_MINIZIP  // (defined(__EMSCRIPTEN__) &&
                         // defined(EMSCRIPTEN_SAVE_SHELL))
            if (isZipFile) {
              if (ImGui::IsItemHovered())
                ImGui::SetTooltip("right click to browse it");
              if (ImGui::GetIO().MouseClicked[1]) {
  ...
  if (isSaveFileDialog || isSelectFolderDialog) {
    ...
    if (isSaveFileDialog) {
      ...
      ImGui::SameLine();
      lastTwoButtonsWidth =
          ImGui::CalcTextSize("Save Cancel").x +
          2.0f * (style.FramePadding.x + style.ItemSpacing.x) +
          style.WindowPadding.x;
      ImGui::PushItemWidth(ImGui::GetContentRegionAvailWidth() -
                           lastTwoButtonsWidth);
      ImGui::InputText("##saveFileNameDialog", &I.saveFileName[0],
                       MAX_FILENAME_BYTES);
      if (ImGui::IsItemHovered() &&
          I.mustFilterSaveFilePathWithFileFilterExtensionString &&
      ...
      ImGui::TextColored(
          ColorSet[Internal::ImGuiCol_Dialog_SelectedFolder_Text], "Folder:");

      // ImGui::Text("Folder:");	// Faster alternative to the 5 lines
      // above

      ImGui::SameLine();
      lastTwoButtonsWidth =
          ImGui::CalcTextSize("Select Cancel").x +
          2.0f * (style.FramePadding.x + style.ItemSpacing.x) +
          style.WindowPadding.x;
      ImGui::PushItemWidth(ImGui::GetContentRegionAvailWidth() -
                           lastTwoButtonsWidth);
      ImGui::InputText("##selectFileNameDialog", &I.saveFileName[0],
                       MAX_FILENAME_BYTES, ImGuiInputTextFlags_ReadOnly);
      ImGui::PopItemWidth();
      ImGui::SameLine();
    }
    ...
    if (selectionButtonPressed) {
      if (isSelectFolderDialog) {
        ...
        if (strlen(I.saveFileName) > 0) {
          bool pathOk = true;
          if (I.mustFilterSaveFilePathWithFileFilterExtensionString &&
            ...
            const bool saveFileNameHasExtension =
                strlen(saveFileNameExtension) > 0;
            //-------------------------------------------------------------------
            FilenameStringVector wExts;
...
const char* Dialog::chooseFileDialog(bool dialogTriggerButton,
                                     const char* directory,
                                     const char* fileFilterExtensionString,
                                     const char* windowTitle,
                                     const ImVec2& windowSize,
  ...
  if (dialogTriggerButton) {
    internal->open = internal->rescan =
        internal->forceSetWindowPositionAndSize = true;
  ...
  if (dialogTriggerButton ||
    ...
    const char* cp = ChooseFileMainMethod(
        *this, directory, false, false, "", fileFilterExtensionString,
        windowTitle, windowSize, windowPos, windowAlpha);
    if (!internal->open) {  // AFAIK this should happen only when user clicks
                            // the close button (but not when he clicks cancel)
...
const char* Dialog::chooseFolderDialog(bool dialogTriggerButton,
                                       const char* directory,
                                       const char* windowTitle,
                                       const ImVec2& windowSize,
                                       const ImVec2& windowPos,
  ...
  if (dialogTriggerButton ||
      (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
    const char* cp =
        ChooseFileMainMethod(*this, directory, true, false, "", "", windowTitle,
                             windowSize, windowPos, windowAlpha);
    if (!internal->open) {
      internal->userHasJustCancelledDialog = true;
#ifdef IMGUI_USE_MINIZIP
      internal->unz.close();
...
const char* Dialog::saveFileDialog(bool dialogTriggerButton,
                                   const char* directory,
  ...
  if (dialogTriggerButton ||
      (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
    const char* cp =
        ChooseFileMainMethod(*this, directory, false, true,
                             startingFileNameEntry, fileFilterExtensionString,
                             windowTitle, windowSize, windowPos, windowAlpha);
    if (!internal->open) {
      internal->userHasJustCancelledDialog = true;
#ifdef IMGUI_USE_MINIZIP
